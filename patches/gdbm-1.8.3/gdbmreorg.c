...

{
  ...
  int  err, len, block_size = dbf->header->block_size;
  ...
  /* A failure here is fatal, since we have already cleaned
   * the "dbf" content. OTOH "dbf->name" should either be the
   * newly create DB (if rename() succeded), or the old one.
   */
  new_dbf = gdbm_open (dbf->name, block_size, GDBM_WRCREAT,
		       fileinfo.st_mode, dbf->fatal_err);
