...
_gdbm_unlock_file (GDBM_FILE dbf)
{
  ...
  switch (dbf->lock_type)
	...
	break;
      ...
      case LOCKING_WIN32:
#if _WIN32
        /* Win32 specific unlock */
        memset(&Ovlp, 0, sizeof(Ovlp));
        UnlockFileEx((HANDLE) _get_osfhandle(dbf->desc), 0, 0xffffffff, 0xffffffff, &Ovlp);
#endif
        break;
...
_gdbm_lock_file (GDBM_FILE dbf)
{
  ...
  if (dbf->read_write == GDBM_READER) {
    /* Win32 specific read-lock */
    OVERLAPPED Ovlp;                \
    memset(&Ovlp, 0, sizeof(Ovlp)); \
    lock_val = LockFileEx((HANDLE) _get_osfhandle(dbf->desc), 0, 0, 0xffffffff, 0xffffffff, &Ovlp); \
  }
  else {
    /* Win32 specific write-lock */
    OVERLAPPED Ovlp;
    memset(&Ovlp, 0, sizeof(Ovlp));
    lock_val = LockFileEx((HANDLE) _get_osfhandle(dbf->desc), LOCKFILE_EXCLUSIVE_LOCK, 0, 0xffffffff, 0xffffffff, &Ovlp);
  }

  if (lock_val) { /* sucess */
    dbf->lock_type = LOCKING_WIN32;
    return 1;
  }
  else { /* failure */
    dbf->lock_type = LOCKING_NONE;
